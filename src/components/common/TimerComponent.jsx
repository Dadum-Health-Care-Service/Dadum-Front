...
export default function Timer({
  ...
  const [time, setTime] = useState(hour * 3600 + minute * 60 + seconds);
  const [timeLeft, setTimeLeft] = useState(hour * 3600 + minute * 60 + seconds);
  const [isRunning, setIsRunning] = useState(false);
  const intervalRef = useRef(null);

  // 타이머 시작/정지 로직
  useEffect(() => {
    if (isRunning && timeLeft > 0 && !pauseTimer) {
    ...
    return () => {
      if (intervalRef.current && !pauseTimer)
        clearInterval(intervalRef.current);
    };
  ...
  return (
    <div className={styles["timer-wrapper"]}>
      <div className={styles["timer-display"]} style={{ fontSize: size }}>
        {formatTime(timeLeft)}
      </div>
