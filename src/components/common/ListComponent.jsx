...
const ListComponent = ({
  ...
  return (
    <ul className={styles[listClasses]} {...props}>
      {children}
    </ul>
  );
};
...
const ListItem = ({
  ...
  return (
    ...
    >
      {(icon || avatar) && (
        <div className={styles["list__item-start"]}>
          {avatar && (
            <div className={styles["list__item-avatar"]}>{avatar}</div>
          )}
          {icon && <div className={styles["list__item-icon"]}>{icon}</div>}
        </div>
      )}

      <div className={styles["list__item-content"]}>
        {primary && (
          <div className={styles["list__item-primary"]}>{primary}</div>
        )}
        {secondary && (
          <div className={styles["list__item-secondary"]}>{secondary}</div>
        )}
        {!primary && !secondary && children}
      </div>

      {action && (
        <div className={styles["list__item-end"]}>
          <div className={styles["list__item-action"]}>{action}</div>
        </div>
      )}
    </li>
  );
...
const ListHeader = ({ children, className = "", ...props }) => {
  return (
    <li className={`${styles["list__header"]} ${className}`} {...props}>
      {children}
    </li>
  );
};
...
const ListDivider = ({ className = "", ...props }) => {
  return (
    <li className={`${styles["list__divider"]} ${className}`} {...props} />
  );
};
  ...
  return (
    <div style={{ padding: "20px", maxWidth: "800px" }}>
      ...
      <div style={{ display: "grid", gap: "20px", marginBottom: "30px" }}>
        ...
        <div>
          <h4>Outlined</h4>
          <ListComponent variant="outlined">
            <ListItem primary="테두리 스타일" secondary="경계선이 있음" />
            <ListItem primary="두 번째 항목" secondary="구분이 명확" />
          </ListComponent>
        </div>
      ...
      <div style={{ marginBottom: "30px" }}>
        <ListComponent direction="horizontal" spacing="small" variant="filled">
          {tags.map((tag) => (
            <ListItem
              key={tag}
