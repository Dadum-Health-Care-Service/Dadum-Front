...
export default function DailySummary() {
  ...
  useEffect(() => {}, [showNoDataModal]);

  useEffect(() => {
    console.log("user", user.usersId);
  ...
  const prepareWatchDataForLLM = (items = []) => {
    ...
    const totalCalories = Math.ceil(
      items.reduce((sum, item) => sum + Number(item.caloriesKcal || 0), 0)
    );
    ...
    return {
      ...
      summary: `오늘 총 걸음수: ${totalSteps.toLocaleString()}보, 소모 칼로리: ${totalCalories}kcal, 이동거리: ${(
        totalDistance / 1000
      ).toFixed(1)}km, 평균 심박수: ${avgHeartRate}bpm`,
      // LLM이 질병 발병 가능성과 운동 추천을 할 수 있도록 더 자세한 정보 제공
      activityLevel:
        totalSteps >= 10000 ? "high" : totalSteps >= 5000 ? "moderate" : "low",
      heartRateStatus:
        avgHeartRate > 100 ? "elevated" : avgHeartRate > 60 ? "normal" : "low",
      calorieBurnStatus:
        totalCalories > 800 ? "high" : totalCalories > 400 ? "moderate" : "low",
    };
  };
  ...
  const handleShowDetails = async () => {
    ...
    try {
      ...
      console.log("currentHealthItems", currentHealthItems);
      ...
      setHealthItems(filteredHealthItems.length ? filteredHealthItems : []);
      setShowDetails(true);
    } catch (e) {
      if (e?.response?.status === 500) {
        setNoDataModalType("server");
  ...
  const loadDaily = async (d) => {
    ...
    try {
      ...
      console.log("[DailySummary] Loading daily data for:", {
        userId: id,
        date: d,
      });
      const res = await GET(
        `/summary/daily?user_id=${id}&date=${d}`,
        {},
        true,
        "ai"
      );
  ...
  const bulkSave = async () => {
    ...
    try {
      ...
      for (const mealId of selectedMeals) {
        ...
        if (
          nextGrams === prev.grams &&
          (!nextLabel || nextLabel === prev.label)
        ) {
          continue;
        }
  ...
  const generateLocalAnalysis = () => {
    ...
    if (
      ...
      return {
        ...
        advice:
          "오늘의 식단을 입력해주세요. 칼로리 페이지에서 사진으로 식사를 기록할 수 있습니다.",
      };
    }
    ...
    if (!meals || meals.length === 0) {
      return {
        ...
        mealSummary: "",
        advice:
          "워치 데이터를 동기화해주세요. 더 정확한 건강 분석을 위해 활동량 데이터가 필요합니다.",
      };
    }
    ...
    try {
      ...
      if (id) {
        ...
        const filteredHealthItems = latestHealthItems.filter((item) => {
          if (!item.recordTime) return false;
          console.log(item.recordTime);
          const dateStr = new Date(item.recordTime).toLocaleDateString(
            "ko-KR",
            {
              timeZone: "Asia/Seoul",
            }
        ...
        if (
          (!meals || meals.length === 0) &&
          (!filteredHealthItems || filteredHealthItems.length === 0)
        ) {
        } else {
  ...
  const calculateActivityStats = () => {
    ...
    const totalCaloriesBurned = Math.ceil(
      healthItems.reduce((sum, item) => sum + Number(item.caloriesKcal || 0), 0)
    );
  ...
  const buildDietTips = () => {
    ...
    if (proteinPct < 15) {
      tips.push(
        "단백질 비율이 낮아요. 달걀·두부·살코기 등 고단백 식품을 추가하세요."
      );
    }
    if (fatPct > 35) {
      tips.push(
        "지방 비중이 높습니다. 튀김·가공육 대신 구이·찜 조리로 바꿔보세요."
      );
    }
    if (fiberG < 20) {
      tips.push(
        "식이섬유가 부족해요. 채소·과일·통곡물을 한 끼에 한 가지씩 추가하세요."
      );
    }
  ...
  const mergeAdviceWithoutDuplicates = (llmLines, dietTips) => {
    ...
    const categorize = (s) => {
      ...
      if (t.includes("지방") && (t.includes("높") || t.includes("과다")))
        return "fat-high";
      if (t.includes("탄수") && (t.includes("높") || t.includes("과다")))
        return "carb-high";
      if (t.includes("단백질") && (t.includes("낮") || t.includes("부족")))
        return "protein-low";
      if (t.includes("칼로리") && t.includes("높")) return "calorie-high";
      if (t.includes("식이섬유") && (t.includes("부족") || t.includes("낮")))
        return "fiber-low";
      return "";
    };
  ...
  const buildActivityChips = () => {
    ...
    chips.push({
      label: "이동거리",
      value: (activityStats.totalDistance / 1000).toFixed(1) + "km",
    });
    chips.push({
      label: "소모 칼로리",
    ...
    if (activityStats) {
      ...
      if (steps < 5000) {
        recommendations.push(
          "매일 30분 빠른 걷기, 주 3회 근력 운동(스쿼트·푸시업·플랭크)을 시작해보세요."
        );
      } else if (steps < 10000) {
        recommendations.push(
          "현재 활동량을 유지하고, 주 2-3회 근력 운동과 유연성 운동을 추가하세요."
        );
      } else {
        recommendations.push(
          "활동량이 충분합니다. 근력·유연성 운동을 병행해 전신 균형을 유지하세요."
        );
      }
    } else {
      // 워치 데이터가 없는 경우에도 기본 가이드 제공
      recommendations.push(
        "매일 30분 이상 걷기 또는 자전거/수영 등 유산소 운동을 실천하세요."
      );
      recommendations.push(
        "주 2-3회 전신 근력 운동(하체·상체·코어)을 병행하세요."
      );
    }
  ...
  const buildRiskHints = () => {
    ...
    if (healthItems && healthItems.length > 0) {
      ...
      const avgHeartRate =
        healthItems.reduce(
          (sum, item) => sum + Number(item.heartRateAvg || 0),
          0
        ) / healthItems.length;
      ...
      if (totalSteps < 5000) {
        hints.push(
          "활동량 부족으로 대사증후군 및 심혈관 질환 위험이 높습니다."
        );
      }
      ...
      if (avgHeartRate > 100) {
        hints.push(
          "평균 심박수가 높아 스트레스나 과도한 운동으로 인한 심혈관 부담이 있을 수 있습니다."
        );
      } else if (avgHeartRate < 60) {
        hints.push(
          "낮은 심박수는 좋은 심혈관 건강을 나타내지만, 극도로 낮다면 의사 상담을 권장합니다."
        );
      }
      ...
      if (calorieBalance > 500) {
        hints.push(
          "섭취 칼로리가 소모량보다 많아 체중 증가 및 당뇨병 위험이 있습니다."
        );
    ...
    if (carbsRatio > 70) {
      hints.push(
        "고탄수화물 식단으로 인한 당뇨병 및 인슐린 저항성 위험이 있습니다."
      );
    }
  ...
  return (
    <div className="report-container">
              ...
              <tbody>
                {healthItems.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td>{(item.distanceKm / 1000).toFixed(2) || 0}</td>
          ...
          <ContainerComponent className="report-section">
            ...
            >
              ...
              <ul style={{ paddingLeft: 18, margin: 0 }}>
                {mergeAdviceWithoutDuplicates(
                  toBulletedLines(dailyText),
                  buildDietTips()
                ).map((line, idx) => (
      ...
      <ModalComponent
        ...
        title={
          noDataModalType === "server" ? "서버 연결 오류" : "워치 정보 없음"
        }
