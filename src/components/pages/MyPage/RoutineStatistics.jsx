...
const dataToCompare = (type, thisData, lastData) => {
  ...
  const reduceFn = (data, key) =>
    data.reduce((acc, val) => acc + (val.routineResult[key] || 0), 0);

  switch (type) {
    case "kcal":
      console.log(thisData, lastData);
      console.log(reduceFn(thisData, "kcal"), reduceFn(lastData, "kcal"));
      return reduceFn(thisData, "kcal") - reduceFn(lastData, "kcal");
    case "growth":
      const keys = ["muscle", "reSet", "volum", "rouTime"];
      return keys.map(
        (key) => reduceFn(thisData, key) - reduceFn(lastData, key)
      );
    case "activity":
      const getUniqueActivities = (data) =>
        new Set(
          data.flatMap((e) =>
            e.routineEndDetails.map((detail) => detail.srName)
          )
        ).size;
  ...
  useEffect(() => {
    ...
    const setInitData = async () => {
      ...
      if (doughnut && Object.keys(doughnut).length > 0) {
        activityTop = Object.entries(doughnut).sort(
          (a, b) => b[1] - a[1]
        )[0][0];
      } else {
        activityTop = "기록된 활동";
      ...
      console.log(kcalCalc, growthCalc, activityCalc);
    };
    ...
    return (
      <div className={styles["stats-logs"]}>
        {originData.map((data, i) => (
          ...
          >
            <Card.Header>{`루틴 ${data.tStart.substring(
              0,
              10
            )} ${data.tStart.substring(11, 16)} ~ ${data.tEnd.substring(
              11,
              16
            )}`}</Card.Header>
            <Card.Body>
      ...
      <div className={styles.stats}>
        <div className={styles["stats-title"]}>
          <h1 style={{ fontWeight: "bold" }}>
            이번{" "}
            <span className={styles["stats-span"]}>
              {toggle === "week" ? "주" : "달"}
            </span>{" "}
            운동 통계
          </h1>
          <div className={styles["stats-toggle"]}>
            <div
              className={`${styles["stats-toggle-item"]} ${
                toggle === "week" ? styles["on"] : styles["off"]
              }`}
              onClick={() => {
                setToggle("week");
              }}
            ...
            <div
              className={`${styles["stats-toggle-item"]} ${
                toggle === "month" ? styles["on"] : styles["off"]
              }`}
              onClick={() => {
                setToggle("month");
              }}
        ...
        <ContainerComponent variant="filled" className={mobile ? "" : "p-3"}>
          <div className="d-flex flex-column" style={{ gap: "1rem" }}>
            ...
            <div
              className={
                mobile
                  ? styles["shadow-overlay-mobile"]
                  : styles["shadow-overlay"]
              }
            >
              <div
                className={
                  mobile ? styles["stats-chart-mobile"] : styles["stats-chart"]
                }
              >
                <div className={styles["chart-title"]}>
                  <div className={styles["title-container"]}>
                    <h2 className={styles["stats-h2"]}>
                      이번 {toggle === "week" ? "주" : "달"}{" "}
                      <span className={styles["stats-span"]}>칼로리</span>{" "}
                      소모량
                    </h2>
                    <h5 className={styles["stats-h5"]}>
                      저번{toggle === "week" ? "주" : "달"} 보다{" "}
                      {compareCalcs.kcal} kcal 더 소모했어요
                    </h5>
                  </div>
                </div>
            ...
            >
              <div
                className={
                  mobile
                    ? styles["stats-chart-mobile"]
                    : styles["stats-chart-side"]
                }
              >
                <div className={styles["chart-title"]}>
                  <h2 className={styles["stats-h2"]}>
                  ...
                  <h5 className={styles["stats-h5"]}>
                    {`이번 ${toggle === "week" ? "주는" : "달은"} ${
                      compareCalcs.activityTop
                    }을 가장 많이 했어요`}
                  </h5>
                </div>
                <DoughnutChart
                ...
                <div className={styles["chart-title"]}>
                  ...
                  <h5 className={styles["stats-h5"]}>
                    저번{toggle === "week" ? "주" : "달"}보다{" "}
                    {compareCalcs.activity}개 더 다양하게 운동했어요
                  </h5>
                </div>
                <DoughnutChart
                  doughnutData={chartData.doughnut}
            ...
            >
              ...
              >
                <div className={styles["chart-title"]}>
                  <h2 className={styles["stats-h2"]}>
                    이번 {toggle === "week" ? "주" : "달"}{" "}
                    <span className={styles["stats-span"]}>운동 퍼포먼스</span>{" "}
                    로그
                  </h2>
                  <h5 className={styles["stats-h5"]}>
                    이번 {toggle === "week" ? "주" : "달"} 운동의 더 상세한
                    데이터를 볼 수 있어요
                  </h5>
                </div>
                <LogSection />
