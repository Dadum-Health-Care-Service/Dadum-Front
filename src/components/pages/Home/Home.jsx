...
const Home = () => {
  ...
  const getExerciseRecommendation = (weather) => {
    ...
    if (temp > 25 && condition.includes("sunny")) {
      ...
      return {
        exercise: "실내 홈트레이닝",
        description:
          "비가 오네요. 집에서 할 수 있는 스트레칭이나 홈트레이닝을 해보세요.",
      };
  ...
  const fetchWeather = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "OpenWeatherMap API 호출 실패, 모의 데이터 사용:",
          apiError
        );
      }
  ...
  const reviews = [
    {
      stars: 5,
      text: "자세 분석 기능이 정말 정확해요. 혼자서도 올바른 자세로 운동할 수 있어서 좋습니다.",
      author: "김OO / 직장인",
    },
    {
      stars: 5,
      text: "운동 초보인데도 세트·휴식까지 자동으로 맞춰줘서 성취감이 생겼습니다.",
      author: "이OO / 대학생",
    },
    {
      stars: 5,
      text: "식단 사진만 찍으면 칼로리가 바로 나와서 편리해요. 건강 관리가 쉬워졌습니다.",
      author: "박OO / 프리랜서",
    },
    {
      stars: 5,
      text: "운동 기록이 자동으로 저장되어서 진행 상황을 한눈에 볼 수 있어요. 동기부여가 됩니다!",
      author: "정OO / 주부",
    },
    {
      stars: 4,
      text: "실시간 자세 피드백 기능이 정말 유용해요. PT 받는 느낌이 들어요.",
      author: "최OO / 회사원",
    },
    {
      stars: 5,
      text: "스마트 워치와 연동되어 종합 건강 리포트를 받아볼 수 있어서 만족스럽습니다.",
      author: "강OO / 자영업자",
    },
  ];
    ...
    if (user && !isLoadingRef.current) {
      ...
      const loadUserData = async () => {
        ...
        try {
          ...
          const routinesRes = await GET("/routine/list", {}, true).catch(
            (err) => {
          ...
          if (Array.isArray(routineArray) && routineArray.length > 0) {
            const formattedRoutines = routineArray.map((routine, index) => {
              ...
              return {
                ...
                time:
                  exerciseCount > 0 ? `${exerciseCount}개 운동` : "운동 없음",
                icon: "",
              };
            });
            ...
            try {
              ...
              const resultsRes = await POST("/routine/result", {}, true).catch(
                (err) => {
              ...
              if (Array.isArray(results) && results.length > 0) {
                ...
                const todayResults = results.filter((r) => {
                  ...
                  const koreanTodayStr = koreanToday
                    .toISOString()
                    .split("T")[0];
                ...
                const validTodayResults = todayResults.filter(
                  (r) => r.setId || r.id
                );
                const uniqueRoutineIds = [
                  ...new Set(validTodayResults.map((r) => r.setId || r.id)),
                ];
                ...
                const koreanTodayStrForTime = koreanTodayForTime
                  .toISOString()
                  .split("T")[0];
                ...
                const totalSeconds = results.reduce((sum, record) => {
                  ...
                  if (
                    endDateStr === todayForTime ||
                    endDateStr === koreanTodayStrForTime
                  ) {
              ...
              try {
                ...
                if (Array.isArray(results) && results.length > 0) {
                  ...
                  const todayResults = results.filter((r) => {
                    ...
                    return (
                      endDateStr === today || endDateStr === koreanTodayStr
                    );
                  });
                  ...
                  const todayForTime = new Date().toISOString().split("T")[0];
                  const koreanTodayForTime = new Date();
                  koreanTodayForTime.setHours(
                    koreanTodayForTime.getHours() + 9
                  );
                  const koreanTodayStrForTime = koreanTodayForTime
                    .toISOString()
                    .split("T")[0];
    ...
    if (hour >= 6 && hour < 12) {
      return {
        ...
        tips: [
          {
            title: "충분한 수분 섭취",
            description: "운동 전 30분 전에 물 200ml 마시기",
          },
          {
            title: "가벼운 워밍업",
            description: "5-10분 가벼운 스트레칭으로 몸 준비",
          },
          {
            title: "충분한 수면",
            description: "7-9시간의 질 좋은 수면으로 회복",
          },
        ],
      };
      ...
      return {
        ...
        tips: [
          {
            title: "점심 후 휴식",
            description: "운동 전 1-2시간 휴식으로 소화",
          },
          {
            title: "스트레칭",
            description: "10-15분 정적 스트레칭으로 근육 이완",
          },
          {
            title: "충분한 수분 섭취",
            description: "운동 후 500ml 이상의 물 마시기",
          },
        ],
      };
    } else {
      return {
        ...
        tips: [
          {
            title: "저녁 운동 후 식사",
            description: "운동 후 30분 내에 단백질 섭취",
          },
        ...
        <Container>
          ...
          {dataStatus && (
            <div className="text-center mb-3">
              <div
                className={`alert ${
                  dataStatus.includes("오류") || dataStatus.includes("실패")
                    ? "alert-danger"
                    : "alert-info"
                }`}
                role="alert"
              ...
              <CardComponent className={styles.challengeCard}>
                <div className={styles.challengeContent}>
                  ...
                  <div className={styles.challengeTarget}>
                    {userStats?.consecutiveDays ?? 0} 일 연속
                  </div>
                      ...
                      {[1, 2, 3, 4, 5, 6, 7].map((day) => (
                        <div
                          ...
                          className={`${styles.dayIndicator} ${
                            day <= (userStats?.consecutiveDays ?? 0)
                              ? styles.dayActive
                              : ""
                          }`}
                        ></div>
                    ...
                    <span className={styles.progressText}>
                      <span style={{ color: "#0A66FF", fontWeight: 500 }}>
                        {userStats?.consecutiveDays === 0
                          ...
                          ? "첫 걸음을 내딛었어요!"
                          : userStats?.consecutiveDays === 2
                          ? "꾸준히 하고 있어요!"
                          : userStats?.consecutiveDays === 3
                          ? "습관이 만들어지고 있어요!"
                          : userStats?.consecutiveDays === 4
                          ? "정말 잘하고 있어요!"
                          : userStats?.consecutiveDays === 5
                          ? "완벽한 일주일이에요!"
                          : userStats?.consecutiveDays >= 6
                          ? "운동 마스터가 되었어요!"
                          : "좋은 습관 유지 중!"}
                      </span>
                    </span>
                    ...
                    <div className={styles.progressBar}>
                      <div
                        ...
                        style={{
                          ...
                          background:
                            "linear-gradient(90deg, #0A66FF, #3B82F6)",
                        }}
                      ></div>
                    </div>
                    <span className={styles.progressText}>
        ...
        <Container>
          ...
          <Row>
            <Col md={6} className="mb-3">
              <CardComponent title="추천 음식" className={styles.nutritionCard}>
            ...
            <Col md={6} className="mb-3">
              <CardComponent title="회복 팁" className={styles.recoveryCard}>
                ...
                <div className={styles.weatherCard}>
                  ...
                  <div className={styles.exerciseRecommendation}>
                    ...
                    <div className={styles.recommendationDescription}>
                      {weather &&
                        getExerciseRecommendation(weather).description}
                    </div>
                  </div>
                </div>
              )}
