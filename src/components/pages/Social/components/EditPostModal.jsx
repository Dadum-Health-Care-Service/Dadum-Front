...
import { updatePost } from "../Api.jsx";
...
export default function EditPostModal({ open, onClose, post, onUpdated }) {
  ...
  const onSave = async () => {
    await updatePost(post.postId, { title, content, imageUrl: imageUrl || null });
    onUpdated?.();
    onClose();
  };

  return (
    <div className="compose-modal open" role="dialog" aria-modal="true" aria-label="게시글 수정">
