...
const FraudStatistics = () => {
  ...
  const getHourlyChartData = () => {
    if (
      !statistics?.transactionsByHour ||
      statistics.transactionsByHour.length === 0
    ) {
  ...
  const getDailyChartData = () => {
    if (
      !statistics?.transactionsByDayOfWeek ||
      statistics.transactionsByDayOfWeek.length === 0
    ) {
      // 백엔드 데이터가 없을 때만 더미 데이터 생성
      ...
      datasets: [
        {
          ...
          borderColor: riskData.map((r) => {
            const level = r[0];
            return (
              colors[level]?.replace("0.8", "1") || "rgba(199, 199, 199, 1)"
            );
          }),
          borderWidth: 2,
        },
    ...
    <div className={styles.statisticsContainer}>
        ...
        <div className={styles.headerContent}>
          <div className={styles.titleSection}>
            ...
            <p className={styles.subtitle}>
              실시간 이상거래 탐지 현황과 AI 모델 성능을 확인하세요
            </p>
          </div>
      ...
      <div className={styles.summaryCards}>
        <StatCard
          ...
          subtitle={
            statistics?.totalTransactions ? "실제 데이터" : "데모 데이터"
          }
          icon="💳"
          trend="up"
          trendValue="+12%"
      ...
      <div className={styles.chartsGrid}>
        <ChartCard
          ...
          description={`24시간 동안의 거래 패턴을 확인하세요. Y축은 거래 건수를 나타냅니다. ${
            statistics?.transactionsByHour ? "(실제 데이터)" : "(데모 데이터)"
          }`}
          loading={loading}
          error={error}
        >
        ...
        <ChartCard
          ...
          description={`AI가 분석한 거래 위험도 분포입니다 ${
            statistics?.riskDistribution ? "(실제 데이터)" : "(데모 데이터)"
          }`}
          loading={loading}
          error={error}
        >
      ...
      <CardComponent
        title="최근 거래 내역"
        className={styles.transactionListCard}
      >
              ...
              {transactions.slice(0, 10).map((transaction) => (
                <div key={transaction.id} className={styles.tableRow}>
                  ...
                  <div className={styles.status}>
                    <span
                      className={
                        transaction.isAnomaly ? styles.anomaly : styles.normal
                      }
                    >
