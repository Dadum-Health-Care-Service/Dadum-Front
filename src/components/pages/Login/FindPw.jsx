        ...
        if(Object.keys(newErrors).length===0){
            ...
            try{
                ...
                if(usersData.usersName === formData.usersName){
                    ...
                    console.log(res2);
                    closeModal();
                    showConfirmModal('비밀번호 찾기 이메일 전송을 성공하였습니다. 로그인으로 이동하시겠습니까?',
                                    '비밀번호 찾기',
                ...
                if(err.request?.responseURL?.includes('/send/password')){
                    closeModal();
                    showBasicModal("비밀번호 찾기 이메일 전송을 실패하였습니다.","네트워크 에러");
                }
                else if(err.response?.status===400) showBasicModal("해당 회원이 존재하지 않습니다.","비밀번호 찾기");
