...
const SHOW_ENGINE_TOGGLE =
  (import.meta.env.DEV ?? false) || (import.meta.env.VITE_SHOW_ENGINE_TOGGLE === "1");
...
function trunkFlex(ls, rs, lh, rh) {
  ...
  const dot = v[0] * 0 + v[1] * -1, n = Math.hypot(...v) + 1e-8;
  return (Math.acos(clamp(dot / n, -1, 1)) * 180) / Math.PI;
}
...
function normAngles(seq) {
  return seq.map(([k, h, t]) => [
    (clamp(k, ANGLE_RANGE.knee.min, ANGLE_RANGE.knee.max) - ANGLE_RANGE.knee.min) /
    ...
    (clamp(t, ANGLE_RANGE.trunk.min, ANGLE_RANGE.trunk.max) - ANGLE_RANGE.trunk.min) /
      (ANGLE_RANGE.trunk.max - ANGLE_RANGE.trunk.min),
  ]);
}
function weightedRMSE(A, B, frameW = null, W = { knee: 0.4, hip: 0.35, trunk: 0.25 }) {
...
function inferViewFromKeypoints(kps) {
  ...
  const ls = by.left_shoulder, rs = by.right_shoulder, lh = by.left_hip, rh = by.right_hip, lk = by.left_knee, rk = by.right_knee;
  if (!ls || !rs || !lh || !rh || !lk || !rk) return { label: "oblique", conf: 0 };
...
const weightsForView = (l) =>
  l === "side" ? { knee: 0.45, hip: 0.4, trunk: 0.15 } :
  l === "front" ? { knee: 0.1, hip: 0.1, trunk: 0.2 } :
                  { knee: 0.35, hip: 0.35, trunk: 0.3 };
function penaltyForIssues(label, issues) {
  const W_side = { kneeAlign: 0.05, trunkLean: 0.65, pelvisTilt: 0.1, asymmetry: 0.2 };
  const W_front = { kneeAlign: 0.55, trunkLean: 0.1, pelvisTilt: 0.25, asymmetry: 0.1 };
  const W_obl = { kneeAlign: 0.3, trunkLean: 0.3, pelvisTilt: 0.2, asymmetry: 0.2 };
...
function assessFrameIssues(kp, exId) {
  ...
  try {
    ...
    const devL = by.left_knee && by.left_ankle && by.left_hip ? distPointToLineNorm(by.left_knee, by.left_ankle, by.left_hip) : 0;
  ...
  try {
    ...
    pelvisTilt = Math.abs((by.left_hip?.y ?? 0) - (by.right_hip?.y ?? 0)) / sh > 0.08;
  } catch {}
  try {
    const kr = kneeAngle(by, "right"), kl = kneeAngle(by, "left");
...
async function ensureSeg(segRef) {
  ...
  if (typeof seg.initialize === "function") try { await seg.initialize(); } catch {}
  segRef.current = seg;
  return seg;
}
async function segOnce(segRef, image, stateRef, intervalMs = 70) {
  const st = stateRef.current, now = performance.now();
...
function renderSeg(ctx, { video, mask, mode, w, h }) {
  ...
  else { ctx.fillStyle = "rgba(36,92,255,.45)"; ctx.fillRect(0, 0, w, h); }
...
function drawHUD(ctx, scoreObj, hasRef, canvas, rec = false, sec = 0) {
  const dpr = window.devicePixelRatio || 1, x = 8*dpr, y = 8*dpr, w = Math.min(260*dpr, canvas.width-16*dpr), h = 36*dpr, r = 12*dpr, p = 10*dpr;
  ...
  const pct = Number.isFinite(avg) ? avg/100 : 0, bx = x+p, by = y+h/2-5*dpr, bw = w-2*p-(rec?60*dpr:0), bh = 10*dpr;
  ...
  const label = hasRef
    ? `정확도 평균: ${Number.isFinite(avg)?avg+"%":"--"}${Number.isFinite(inst) ? ` (순간 ${inst}%)` : ""}`
    : "레퍼런스 필요";
  ctx.fillText(label, bx, y+h/2);
  if (rec) { const rx = x+w-50*dpr, ry = y+h/2; ctx.fillStyle="#EF4444"; ctx.beginPath(); ctx.arc(rx,ry,5*dpr,0,Math.PI*2); ctx.fill();
...
async function pickDeviceIdForFacing(facingHint = "environment") {
  try {
    ...
    const back = /back|rear|environment|후면/i, front = /front|user|전면/i;
    if (facingHint === "environment") return videos.find((v) => back.test(v.label))?.deviceId ?? videos.at(-1)?.deviceId ?? videos[0].deviceId;
    return videos.find((v) => front.test(v.label))?.deviceId ?? videos[0].deviceId;
  } catch { return null; }
}
async function getStreamWithFacing(facing = "user") {
  try {
    return await navigator.mediaDevices.getUserMedia({ video: { facingMode: { ideal: facing }, width: { ideal: 1280 }, height: { ideal: 720 } }, audio: false });
  } catch {
    const deviceId = await pickDeviceIdForFacing(facing); if (!deviceId) throw new Error("No camera");
    return await navigator.mediaDevices.getUserMedia({ video: { deviceId: { exact: deviceId }, width: { ideal: 1280 }, height: { ideal: 720 } }, audio: false });
  }
}
async function getStreamFromVideoFile(fileOrUrl) {
  ...
  if (vid.paused) await new Promise((r) => { const on = () => { vid.removeEventListener("playing", on); r(); }; vid.addEventListener("playing", on, { once: true }); vid.play().catch(()=>{}); });
  ...
  const cleanup = () => { try { vid.pause(); vid.src = ""; vid.remove(); } catch {} if (typeof fileOrUrl !== "string") try { URL.revokeObjectURL(url); } catch {} };
 ...
 * ========================= */
const EX_DB = [
  { id:"squat",  name:"스쿼트", joints:["knee","hip","trunk"], desc:"하지 굴곡/신전 패턴.", cues:["무릎=발끝","가슴 열기","발 전체로 밀기"], proto:{ kneeROM:110, hipROM:90, trunkROM:20 } },
  { id:"lunge",  name:"런지",   joints:["knee","hip","trunk"], desc:"편측 지지 균형/ROM.",  cues:["앞무릎 발끝 넘지 않기","골반 정렬"],                    proto:{ kneeROM:100, hipROM:80, trunkROM:25 } },
  { id:"pushup", name:"푸시업", joints:["trunk"],             desc:"코어-상지 연계.",      cues:["팔꿈치 45°","몸 일직선"],                               proto:{ kneeROM:10,  hipROM:15, trunkROM:10 } },
  { id:"situp",  name:"싯업",   joints:["trunk","hip"],        desc:"복부 중심 굴곡.",      cues:["턱 가볍게","반동 금지"],                                proto:{ kneeROM:30,  hipROM:60, trunkROM:45 } },
  { id:"plank",  name:"플랭크", joints:["trunk"],             desc:"정적 코어 안정.",      cues:["복부/둔근 수축","허리 중립"],                           proto:{ kneeROM:10,  hipROM:10, trunkROM:8  } },
];
function classifyByDB({ kneeROM, hipROM, trunkROM }) {
  ...
  for (const ex of EX_DB) {
    const p = ex.proto;
    const d = N(kneeROM - p.kneeROM, 120) ** 2 + N(hipROM - p.hipROM, 120) ** 2 + N(trunkROM - p.trunkROM, 90) ** 2;
...
export default function PoseAccuracyMVP() {
  ...
  useEffect(() => {
    (async () => {
      try { const r = await fetch(`${API_BASE}/docs`, { method: "HEAD" });
        if (r.ok) { setServerUp(true); if (!SHOW_ENGINE_TOGGLE) setEngine("yolo"); }
        else { setServerUp(false); setEngine("local"); }
      } catch { setServerUp(false); setEngine("local"); }
    })();
  }, []);

  /* 세그 모드 바뀔 때 이전 마스크 캐시 초기화 */
  useEffect(() => {
    if (segStateRef.current) { segStateRef.current.last = null; segStateRef.current.lastAt = 0; }
  }, [segMode]);
  ...
  /* Ingest keypoints */
  const ingestKeypoints = useCallback((named, drawCtxScale = null, drawCtx = null) => {
    if (drawCtx && drawCtxScale) {
      const { sx, sy } = drawCtxScale;
      drawSkel(drawCtx, named.map((k) => ({ ...k, x: k.x * sx, y: k.y * sy })));
    }
    ...
    S.t.push(now); S.knee.push(kneeA); S.hip.push(hipA); S.trunk.push(trunkA);
    if (issues.kneeAlign) S.issues.kneeAlign++; if (issues.trunkLean) S.issues.trunkLean++;
    if (issues.pelvisTilt) S.issues.pelvisTilt++; if (issues.asymmetry) S.issues.asymmetry++; S.issues.total++;
  }, []);
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      if (mockMode) {
        ...
        s = stream; mockCleanupRef.current = cleanup; mockSrcRef.current = vid;
      } else {
        s = await getStreamWithFacing(camFacing); mockCleanupRef.current = () => {}; mockSrcRef.current = null;
      }
      streamRef.current = s;
      ...
      sessionRef.current = { start: performance.now(), t: [], score: [], knee: [], hip: [], trunk: [], issues: { kneeAlign:0, trunkLean:0, pelvisTilt:0, asymmetry:0, total:0 } };
  ...
  const stop = useCallback(() => {
    ...
    try { mockCleanupRef.current?.(); } catch {} try { mockSrcRef.current = null; } catch {}
    ...
    try {
      ...
      const I = S.issues || { total: 0 }, ratio = (n) => (I.total ? n / I.total : 0);
      const fb = [];
      if (ratio(I.kneeAlign)  > 0.35) fb.push({ id:"kneeAlign",  title:"무릎-발목 정렬이 자주 무너져요", detail:"무릎이 발목-고관절 라인에서 벗어납니다.", fix:"발끝과 같은 방향으로 보냅니다." });
      if (ratio(I.trunkLean)  > 0.35) fb.push({ id:"trunkLean",  title:"몸통 전경사가 과해요", detail:"몸통이 자주 과도하게 숙여집니다.", fix: exRef.current?.id==="hinge" ? "힌지는 허리 중립 유지." : "가슴 열고 코어 안정화." });
      if (ratio(I.pelvisTilt) > 0.35) fb.push({ id:"pelvisTilt", title:"골반 수평 흔들림", detail:"좌우 힙 높이 차 큼.", fix:"양발로 바닥을 균등히 눌러 지지." });
      if (ratio(I.asymmetry) > 0.35) fb.push({ id:"asymmetry",  title:"좌우 비대칭 큼", detail:"양쪽 무릎 굽힘 차 큼.", fix:"약한 쪽에 체중을 더 실어 천천히." });

      setResult({ score: avg, reps, rom, series: null, exName: exRef.current?.name || null, feedback: fb.slice(0,3) });
  ...
  const switchCamera = useCallback(async (nextFacing) => {
    ...
    try {
      ...
      try { streamRef.current?.getTracks?.().forEach((t) => t.stop()); } catch {}
      streamRef.current = newStream; v.srcObject = newStream;
      await v.play().catch(() => {}); if (v.readyState < 2 || !v.videoWidth) await once(v, "loadedmetadata");
      setStatus("실시간 분석 중");
    } catch (e) { console.error(e); setStatus("카메라 전환 실패"); }
  }, [mockMode]);
  ...
  const computeInstantScore = useCallback(() => {
    ...
    const wLive = resampleSeq(ws.map((w) => [w, 0, 0]), CFG.REF_N).map((x) => x[0]);
    const wComb = wLive.map((wl, i) => Math.min(wl, refSigRef.current.w?.[i] ?? 1));
    const WV = weightsForView(viewRef.current.label);
    const Wmix = blendW(W_REF.current, WV, viewRef.current.label === "front" ? 0.75 : 0.5);
    ...
    const penaltyScale = viewRef.current.label === "front" ? 0.4 : viewRef.current.label === "side" ? 0.25 : 0.33;
  ...
  const loop = async () => {
    ...
    const step = async () => {
      ...
      if (v.readyState >= 2) {
        if (mockMode && mockSrcRef.current && mockSrcRef.current.paused) mockSrcRef.current.play().catch(()=>{});
        const vw = v.videoWidth, vh = v.videoHeight;
        const scale = Math.min(c.width / vw, c.height / vh), dw = vw * scale, dh = vh * scale;

        ctx.clearRect(0, 0, c.width, c.height);
        ctx.save();

        const flip = mockMode ? 1 : camFacing === "user" ? -1 : 1;
        ctx.translate(c.width / 2, c.height / 2); ctx.scale(flip, 1); ctx.translate(-dw / 2, -dh / 2);

        // Segmentation (overlay만 UI 제공)
        let segMask = null;
        if (segMode !== "off" && engine === "local") {
          try { segMask = (await segOnce(segRef, v, segStateRef, 70))?.segmentationMask || null; } catch {}
        }
        if (segMask && segMode !== "off") {
          ctx.save();
          renderSeg(ctx, { video: v, mask: segMask, mode: "overlay", w: dw, h: dh });
          if (segMode === "overlay") { ctx.globalCompositeOperation = "destination-over"; ctx.drawImage(v, 0, 0, dw, dh); ctx.globalCompositeOperation = "source-over"; }
          ctx.restore();
        } else {
        ...
        if (engine === "local") {
          if (now - lastInfer.current > 1000 / CFG.LIVE_FPS) {
            ...
            try {
              ...
              const kp = poses?.[0]?.keypoints; if (kp) ingestKeypoints(kp, { sx: dw / vw, sy: dh / vh }, ctx);
            } catch {}
          }
        } else {
          if (yoloImgRef.current) ctx.drawImage(yoloImgRef.current, 0, 0, dw, dh);
          if (!yoloBusy.current && now - lastInfer.current > 1000 / CFG.YOLO_FPS) {
            lastInfer.current = now; yoloBusy.current = true;
            try {
              const ofs = offscreen.current, pctx = ofs.getContext("2d");
              pctx.drawImage(v, 0, 0, ofs.width, ofs.height);
              const dataURL = ofs.toDataURL("image/jpeg", 0.8);
              const res = await fetch(`${API_BASE}/ai/pose`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ image: dataURL }) }).then((r) => r.json());
              if (res?.image) { const img = new Image(); img.onload = () => (yoloImgRef.current = img); img.src = res.image; }
              if (Array.isArray(res?.keypoints) && res.keypoints.length >= 17) ingestKeypoints(yoloKeypointsToNamed(res.keypoints));
            } catch {} finally { yoloBusy.current = false; }
          }
        }
      ...
      if (refSigRef.current) {
        ...
        if (Number.isFinite(inst)) {
          const S = sessionRef.current;
          S.score.push(inst);
          const avg = Math.round(S.score.reduce((a, b) => a + b, 0) / S.score.length);
          hudScore = { inst, avg };
  ...
  async function onRefFile(e) {
    ...
    const url = URL.createObjectURL(file); await processReference(url, file.name); try { URL.revokeObjectURL(url); } catch {}
  }
  async function processReference(url, displayName = "reference.mp4") {
    ...
    // 30FPS
    const dt = 1 / 30, duration = v.duration || 0; let t = 0, lastUI = 0;
    const seq = [], ws = [];

    const proc = document.createElement("canvas"); proc.width = c.width; proc.height = c.height;
    ...
    while (t <= duration) {
      ...
      try {
        ...
        if (s?.segmentationMask) {
          renderSeg(pctx, { video: v, mask: s.segmentationMask, mode: "person", w: proc.width, h: proc.height });
          inferInput = proc;
        } else { pctx.drawImage(v, 0, 0, proc.width, proc.height); inferInput = proc; }
      } catch { pctx.drawImage(v, 0, 0, proc.width, proc.height); inferInput = proc; }
      ...
      t += dt; const now = performance.now();
      if (now - lastUI > 80) { setRefProg((p) => ({ ...p, pct: Math.min(100, Math.round((t / duration) * 100)) })); lastUI = now; }
    }

    const s1 = emaSmooth(seq, 0.7), s2 = resampleSeq(s1, CFG.REF_N), nA = normAngles(s2);
    const w2 = resampleSeq(ws.map((w) => [w, 0, 0]), CFG.REF_N).map((x) => x[0]);
    ...
    <div className="pose-page">
      ...
      <section className="card">
        <div className="card-head">
          <div className={`status ${isRunning ? "is-on" : ""}`}>상태: {status}</div>
          <Segment options={segOptions} value={segMode} onChange={setSegMode} title="세그" />
          {SHOW_ENGINE_TOGGLE && serverUp && (
            <Segment options={engineOptions} value={engine} onChange={changeEngine} title="엔진" />
          )}
          <Segment options={camOptions} value={camFacing} onChange={(v) => switchCamera(v)} disabled={refProg.running || mockMode} title="카메라" />
          <div className="toolbar-spacer" />
          <label className="toggle" title="녹화영상으로 테스트">
            <span>녹화영상</span>
            <input type="checkbox" checked={mockMode} onChange={(e) => setMockMode(e.target.checked)} />
          </label>
          <label className={`file-btn ${!mockMode ? "disabled" : ""}`} title="비디오 파일 선택">
            <input type="file" accept="video/*" disabled={!mockMode} onChange={(e) => setMockFile(e.target.files?.[0] || null)} />
            <span>파일 선택</span>
            <span className="file-name">{mockFile?.name || "선택된 파일 없음"}</span>
          </label>
          <div className="chip" title="현재 상태">
            seg:{segMode} · eng:{engine}{serverUp ? "" : " (srv↓)"} · cam:{camFacing} · mock:{String(mockMode)}
          </div>
        </div>
        ...
        <button type="button" className={`cta ${isRunning ? "stop" : ""}`} onClick={(e) => { e.preventDefault(); (isRunning ? stop : start)(); }} disabled={refProg.running}>
      ...
      <section className="card">
        ...
        {refProg.running ? (
          <div>
            ...
            <div className="progress"><div className="progress-fill" style={{ width: `${refProg.pct}%` }} /></div>
          </div>
        ) : (
        ...
        <p className="hint">레퍼런스를 업로드하면 자동 매칭되어 <b>설명/분석 관절</b>이 아래에 표시됩니다.</p>
      </section>
      ...
      {options.map((opt) => {
        ...
        return (
          <button
            ...
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); if (!disabled) onChange?.(opt.value); }}
            disabled={disabled}
            title={opt.title || ""}
          >
            {opt.label}
